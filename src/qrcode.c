...
void qrcode_insert_information(QRCode* self, Array* information, size_t current_idx) {
    ...
    for(size_t i = 0; i < information->size; i++) {
        current_idx = get_next_available_idx(current_idx, &matrix);
...
static Array* process_information(
    ...
    EncodingMode encoding_mode) {
        size_t codewords_count = get_number_codewords(qrcode->version, encoding_mode);
        Array* binary_encoding_mode = get_encoding_mode(encoding_mode);
        // TODO: change length depending on the encoding mode
        Array* binary_count_indicator = encode_int_to_binary(strlen(string), 9);
        Array* binary_information = array_alloc(codewords_count * 8);
        Array* alph_encoding = encode_alphanumeric_string(string);
        ...
        pad_codewords(binary_information);
...
QRCode* qrcode_generate(
    ...
    Array* format_version = compute_format(
        get_error_correction_level(correction_mode),
        mask_pattern
    );
    ...
    reserve_all_patterns(&mask_matrix);
    ...
    for(size_t i = 0; i < information_poly->degree; i++) {
        Array* int_to_bin = encode_int_to_binary(information_poly->values[i], 8);
    ...
    set_all_finders(&value_matrix);
    set_timings_n_dark(&value_matrix);
